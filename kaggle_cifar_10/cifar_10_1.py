...
各自扮演着不同的角色，但它们经常一起使用以构建高效的数据加载和预处理管道。

torchvision.datasets.ImageFolder
torchvision.datasets.ImageFolder 是一个用于从文件夹中加载图像数据的类。它假设每个子文件夹包含同一类别的图像。
例如，如果你有一个名为 "cat" 的文件夹，其中包含所有猫的图片，以及一个名为 "dog" 的文件夹，其中包含所有狗的图片，
ImageFolder 会自动将这些图片与相应的标签关联起来。
